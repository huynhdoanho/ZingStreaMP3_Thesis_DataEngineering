...
create_external_table_auth_events = RedshiftSQLOperator(
    ...
    sql=f"""
        ...
        STORED AS PARQUET
        LOCATION 's3://zingstreamp3/data/auth_events/year={year}/month={month}/day={day}';
        """,
    dag=dag
)
...
create_external_table_listen_events = RedshiftSQLOperator(
    ...
    sql=f"""
        ...
        STORED AS PARQUET
        LOCATION 's3://zingstreamp3/data/listen_events/year={year}/month={month}/day={day}';
    """,
    dag=dag
)
...
create_external_table_page_view_events = RedshiftSQLOperator(
    ...
    sql=f"""
        ...
        STORED AS PARQUET
        LOCATION 's3://zingstreamp3/data/page_view_events/year={year}/month={month}/day={day}';
        """,
    dag=dag
)
...
create_external_table_status_change_events = RedshiftSQLOperator(
    ...
    sql=f"""
        ...
        STORED AS PARQUET
        LOCATION 's3://zingstreamp3/data/status_change_events/year={year}/month={month}/day={day}';
    """,
    dag=dag
)
...
clean_realtime_data = RedshiftSQLOperator(
    task_id='clean_realtime_data',
    redshift_conn_id="redshift_zingstreamp3_realtime",
    sql="""
            DELETE FROM zingstreamp3.listen_events
            WHERE EXTRACT(DAY FROM ts) = EXTRACT(DAY FROM sysdate - 1)
        """,
    dag=dag
)
